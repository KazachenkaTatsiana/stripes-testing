  ...
  it('C358998 Data Import Updates should add 035 field from 001/003, if it is not HRID (folijet)',
    { tags: [TestTypes.criticalPath, DevTeams.folijet] }, () => {
      ...
      cy.wrap(fieldsContent).each(row => {
        cy.wait(8000);
        FileDetails.checkStatusInColumn(FileDetails.status.created, FileDetails.columnName.srsMarc, row.rowNumber);
        FileDetails.checkStatusInColumn(FileDetails.status.created, FileDetails.columnName.instance, row.rowNumber);
      });
      ...
      cy.wrap(fieldsContent).each(row => {
        cy.wait(8000);
        FileDetails.checkStatusInColumn(FileDetails.status.updated, FileDetails.columnName.srsMarc, row.rowNumber);
        FileDetails.checkStatusInColumn(FileDetails.status.updated, FileDetails.columnName.instance, row.rowNumber);
      });
